...
# -*- coding: utf-8 -*-
"""
NAO Robot REST API Server (Laptop Version)
Run this on your laptop that's connected to the same WiFi as your NAO robot.

Requirements:
    pip install paramiko flask flask-cors pillow
...
if sys.version_info[0] < 3:
    print("This script requires Python 3. For Python 2.7, use the NAO-native version.")
    sys.exit(1)
...
try:
    ...
    print("  pip install paramiko flask flask-cors pillow")
    sys.exit(1)

# ==================== Configuration ====================
NAO_IP = "172.18.16.35"  # Change this to your NAO's IP
NAO_PORT = 9559
SSH_USERNAME = "nao"
SSH_PASSWORD = "nao"
SERVER_PORT = 5000
...
class NAOController:
    ...
    def _execute_naoqi(self, code, timeout=30):
        ...
        # Wrap the code with NAOqi imports
...
def api_root():
    return jsonify({
        "message": "NAO Robot REST API Server (Laptop Version)",
        "version": "1.0.0",
        "nao_ip": NAO_IP
    })
...
def camera_frame():
    ...
    # Camera requires more complex handling, return placeholder for now
    return jsonify({"error": "Camera not implemented in laptop version"}), 501
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("  NAO Robot REST API Server (Laptop Version)")
    print("=" * 60)
    print(f"\n  NAO Robot IP: {NAO_IP}")
    ...
    # Auto-connect to NAO on startup
